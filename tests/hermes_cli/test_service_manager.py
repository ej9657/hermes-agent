...
def _assert_s6_event_mode(path) -> None:
    ...
    if sys.platform == "darwin":
        # APFS strips setgid on ordinary unprivileged directories. Linux
        # container hosts preserve the full s6 target mode and remain exact.
        assert mode in {0o3730, 0o1730}
    else:
        assert mode == 0o3730
